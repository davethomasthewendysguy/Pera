...
function skeleton_registerstyles() {
    ...
    $stylesheets .= wp_register_style('theme', get_stylesheet_directory_uri().'/style.css', array(), $version, 'screen, projection');
     //$stylesheets .= wp_register_style('supersized', get_stylesheet_directory_uri().'/css/supersized.css', array(), $version, 'screen, projection');
     //$stylesheets .= wp_register_style('shutter_theme', get_stylesheet_directory_uri().'/theme/supersized.shutter.css', array(), $version, 'screen, projection');
	...
	wp_enqueue_style('shutter_theme');
}
...
if ( !function_exists( 'skeleton_header_scripts' ) ) {
	...
	function skeleton_header_scripts() {
		...
		$javascripts .= wp_enqueue_script('easing',get_template_directory_uri()."/javascripts/jquery.easing.min.js",array('jquery'),'1.2.3',true);
		$javascripts .= wp_enqueue_script('custom-script',get_template_directory_uri()."/javascripts/custom.js",array('jquery'),'1.2.3',true);
		$javascripts .= wp_enqueue_script('supersized',get_template_directory_uri()."/javascripts/supersized.3.2.7.min.js",array('jquery'),'1.2.3',true);
		$javascripts .= wp_enqueue_script('shutter_theme',get_template_directory_uri()."/theme/supersized.shutter.min.js",array('jquery'),'1.2.3',true);
...
if ( !function_exists( 'skeleton_main_menu' ) ) {
	...
	function skeleton_main_menu() {
		...
		$defaults = array(
			...
			'link_before'     => '',
			'link_after'      => '',
			'items_wrap'      => '<ul id="%1$s" class="%2$s"><a href="#"></a>%3$s</ul>'
		);
...
function skeleton_widgets_init() {
	...
	// Area 6, located in the footer. Empty by default.
	register_sidebar( array(
		'name' => __( 'Bottom Fixed Widget Area', 'smpl' ),
		'id' => 'bottom-widget-area',
		'description' => __( 'Bottom Fixed Widget Area', 'smpl' ),
		'before_widget' => '<div class="%1$s">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
