...
class Woo_Layout {
	...
	function get_layout_info () {
		...
		if ( is_array( $woo_layouts ) && is_array( $woo_options ) ) {
			...
			$data['container_class'] = $container_class;
			$data['one_percent'] = ( $container_width / 100 );
			$data['gutter'] = 2.0833333333333333333333333333333;
	...
	function load_dynamic_css () {
		...
		if ( isset( $_GET['woo-layout-css'] ) && $_GET['woo-layout-css'] == 'load' ) {
			...
			$msieConstant = 0.0531914893617024;
			...
			if ( ! ( is_ie( '8.0' ) || is_ie( '7.0' ) || is_ie( '6.0' ) ) )
			...
			// Doms: Perform width-matching to account form the compounding of %. 

			if (preg_match("/^three-/", $layout)) {
				$width_main = floor(absint($width_main) * pow(absint($width_main) + absint($width_secondary), -1) * 100);
				$width_primary = floor(absint($width_secondary) * pow(absint($width_main) + absint($width_secondary), -1) * 100);
			}


			if ( $width_maincontainer ) {
				if ( $layout == 'three-col-middle' ) {
					$width_maincontainer -= $gutter;
				}
				$css .= '.' . $layout . $container_class . ' #main-sidebar-container { width: ' . $width_maincontainer . '%' . '; *width: ' . ($width_maincontainer - $msieConstant) . '%;}' . "\n";
			}
			if ( $width_main ) {
				...
				$css .= '.' . $layout . $container_class . ' #main-sidebar-container #main { width: ' . $width_main . '%' . '; *width: ' . ($width_main - $msieConstant) . '%;}' . "\n";
			}
			if ( $width_primary ) {
				$css .= '.' . $layout . $container_class . ' #main-sidebar-container #sidebar { width: ' . $width_primary . '%' . '; *width: ' . ($width_primary - $msieConstant) . '%;}' . "\n";
				
				if ( $layout == 'three-col-left' ) {
					$css .= '.' . $layout . $container_class . ' #main-sidebar-container #sidebar { padding-right: ' . $gutter . '%' . '; *padding-right: ' . ($gutter - $msieConstant) . '%;}' . "\n";
				}
				if ( $layout == 'three-col-right' ) {
					$css .= '.' . $layout . $container_class . ' #main-sidebar-container #sidebar { padding-left: ' . $gutter . '%' . '; *padding-left: ' . ($gutter - $msieConstant) . '%;}' . "\n";
				}
			}
			if ( $width_secondary ) { $css .= '.' . $layout . $container_class . ' #sidebar-alt { width: ' . $width_secondary . '%' . '; *width: ' . ($width_secondary - $msieConstant) . '%;}' . "\n"; }
