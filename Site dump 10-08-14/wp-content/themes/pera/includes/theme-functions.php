...
if ( function_exists('wp_nav_menu') ) {
	...
	register_nav_menus( array( 'sub-menu' => __( 'Sub Menu', 'woothemes' ) ) );
	register_nav_menus( array( 'change-location' => __( 'Change Location', 'woothemes' ) ) );
}
...
if (!function_exists('woo_get_the_metabox_settings')) {
	function woo_get_the_metabox_settings($id = '') {
		global $post;

		if ( ! is_numeric( $id ) )
			$id = $post->ID;

		/* Get post meta and store array in a variable. */
		$page_metabox_settings = get_post_meta($id,NULL,false);

		/* Array sanity check. */
		if ( ! is_array( $page_metabox_settings ) )
			return;

		foreach ( $page_metabox_settings as $key => $value ) {
			if ( is_array( $value ) && isset( $value[0] ) ) {
				$page_metabox_settings[$key] = $value[0];
			} else {
				$page_metabox_settings[$key] = $value;
			}
		}

		return $page_metabox_settings;
	}
}

if (!function_exists('woo_get_the_alias')) {
	function woo_get_the_alias($id = '') {
		global $post;
		...
		/* Get post meta and store array in a variable. */
		$page_metabox_settings = woo_get_the_metabox_settings($id);

		/* Array sanity check. */
		if ( ! is_array( $page_metabox_settings ) )
			return;

		extract( $page_metabox_settings, EXTR_OVERWRITE );

		return ( isset( $_page_title_alias ) && '' != $_page_title_alias) ? $_page_title_alias : get_the_title($id);
	}
}

if (!function_exists('woo_get_the_parent_title')) {
	function woo_get_the_parent_title($id = '') {
		global $post;
		...
		return ( 0 == (int) $post->post_parent ) ? get_the_title() : get_the_title( $post->post_parent );
	}
}

if (!function_exists('woo_get_the_parent_alias')) {
	function woo_get_the_parent_alias($id = '') {
		global $post;
		...
		return woo_get_the_alias( $id );
	}
}

if (!function_exists('woo_the_alias')) {
	function woo_the_alias($id = '') {
		echo woo_get_the_alias($id);
	}
}

if (!function_exists('woo_get_the_title')) {
	function woo_get_the_title($args = array(), $id = '') {
		global $post;
		global $woo_options;
		...
		/* Array sanity check. */
		if ( ! is_array( $woo_options ) || ! is_array( $args ) || ! is_array( $page_metabox_settings ) )
			return;
		...
		if ( '' != $use_option && array_key_exists( $use_option, $woo_options ) ) {
			return $woo_options[$use_option];
		} else {
			switch( $_page_metabox_set_page_title_masking_precedence ) {
				case "alias":
					return woo_get_the_alias($id);
					break;
				case "parent":
					return woo_get_the_parent_alias($id);
					break;
				case "none":
				default:
...
if (!function_exists('woo_the_title')) {
	function woo_the_title($args = array(), $id = '') {
		...
		/* Array sanity check. */
		if ( ! is_array($woo_options) || ! is_array($args) )
			return;

		/* Precedence: alias, parent, none */
		$defaults = array(
		...
		/* Provide variables, skipping extant ones. */
		extract( $args, EXTR_OVERWRITE);
		extract( $page_metabox_settings, EXTR_OVERWRITE );

		if ( ! isset( $_page_metabox_set_page_title_masking_precedence ) )
			$_page_metabox_set_page_title_masking_precedence = "none";

		if ('true' == $_page_metabox_display_page_titles)
			echo $before . woo_get_the_title() . $after;
	}
}
...
/* Echo custom CSS */
/*-----------------------------------------------------------------------------------*/
if (!function_exists('woo_page_css')) {
	function woo_page_css() {
		...
		/* Get post meta and store array in a variable. */
		$page_options = get_post_meta($post->ID,NULL,false);

		$content = '<style type="text/css">';
		$content .= stripslashes($page_options['_page_custom_css'][0]);
		$content .= '</style>';
...
if (!function_exists('woo_page_js')) {
	function woo_page_js() {
		...
		$content .= stripslashes($page_options['_page_custom_js'][0]);
		$content .= '</script>';
