	...
	if($schedules) {
		...
		foreach($schedules as $schedule) {
		    ...
		    <tr id='adrotateindex' class='<?php echo $class; ?>'>
				...
				<td><a href="<?php echo admin_url('/admin.php?page=adrotate-schedules&view=edit&schedule='.$schedule->id);?>"><?php echo stripslashes(html_entity_decode($schedule->name)); ?></a><?php if($schedule->spread == 'Y') { ?><span style="color:#999;"><br /><span style="font-weight:bold;">Spread:</span> Max. <?php echo $schedule->hourimpressions; ?> <?php _e('impressions per hour', 'adrotate'); ?></span><?php } ?></td>
