...
function adrotate_ad($banner_id, $individual = true, $group = 0, $block = 0, $site = 0) {
	...
	if($banner_id) {
		if($individual == true) {
			...
			$selected = array($banner->id => 0);			
			$schedules = array('Already checked when choosing group.');
		}
...
function adrotate_inject_posts($post_content) { 
	...
	if(is_page() OR is_single()) {
		...
		$groups = $wpdb->get_results("SELECT `id`, `geo`, `cat`, `cat_loc`, `cat_par`, `page`, `page_loc`, `page_par`, `wrapper_before`, `wrapper_after` FROM `".$prefix."adrotate_groups` WHERE `page_loc` > 0 OR `cat_loc` > 0 ORDER BY `id` ASC;");
		...
		if(is_page()) {
			foreach($groups as $group) {
				...
				if(in_array($post->ID, $pages)) {
					$group_array[] = array('group' => $group->id, 'location' => $group->page_loc, 'paragraph' => $group->page_par, 'geo' => $group->geo, 'before' => $group->wrapper_before, 'after' => $group->wrapper_after);
				}
			}
		}
		...
		if(is_single()) {
			...
			foreach($groups as $group) {
				...
				if(in_array($category[0]->cat_ID, $categories)) {
					$group_array[] = array('group' => $group->id, 'location' => $group->cat_loc, 'paragraph' => $group->cat_par, 'geo' => $group->geo, 'before' => $group->wrapper_before, 'after' => $group->wrapper_after);
				}
			}
		}
		...
		if(count($group_array) > 0) {
			...
			if($ads) {
				foreach($ads as $ad) {
					...
					if($adrotate_config['enable_geo'] > 0 AND $group_array[$group_select]['geo'] == 1) {
						$selected = adrotate_filter_location($selected, $ad);
					}
				}
				unset($ads);
		...
		if(count($matches) > 0) {
			foreach($matches as &$value) {
				if(preg_match('/<img[^>]+class=\"(.+?)\"[^>]*>/i', $value[0], $regs)) {
				    $result = $regs[1]." adblock";
					$str = str_replace('class="'.$regs[1].'"', 'class="'.$result.'"', $value[0]);	    
				} else {
					$str = str_replace('<img ', '<img class="adblock" ', $value[0]);
				}
				$banner_output = str_replace($value[0], $str, $banner_output);
				unset($value, $regs, $result, $str);
			}
...
function adrotate_custom_head() {
	...
	$groups = $wpdb->get_results("SELECT `id`, `modus`, `gridrows`, `gridcolumns`, `adwidth`, `adheight`, `admargin`, `admargin_bottom`, `admargin_left`, `admargin_right`, `adspeed` FROM `".$wpdb->prefix."adrotate_groups` WHERE `name` != '' ORDER BY `id` ASC;");
	if($groups) {
		$array_js = $array_css = false;
		foreach($groups as $group) {
			if($group->modus == 0 OR $group->modus == 1) { // Single ad or Dynamic group
				if($group->adwidth != 'auto') {
					...
					$width = "min-width:".$group->admargin_left."px; max-width:".$width_sum."px;";
				} else {
					$width = "width:auto;";
				}
				...
				$array_css[] = "\t.g-".$group->id." { margin:".$group->admargin."px ".$group->admargin_right."px ".$group->admargin_bottom."px ".$group->admargin_left."px; ".$width." ".$height." }\n";
			...
			if($group->modus == 2) { // Block group
				...
				$array_css[] = "\t.b-".$group->id." { margin:".$group->admargin."px ".$group->admargin_right."px ".$group->admargin_bottom."px ".$group->admargin_left."px; }\n";
	...
	if($adrotate_config['adblock'] == 'Y' OR isset($output_css) OR $adrotate_config['widgetpadding'] == "Y") {
		...
		if(($adrotate_config['adblock'] == 'Y' AND !is_user_logged_in()) OR ($adrotate_config['adblock'] == 'Y' AND $adrotate_config['adblock_loggedin'] == "Y" AND is_user_logged_in())) {
			...
			$output .= "#adblock_notice_content { position:fixed; background:#fff; border-radius:15px; -webkit-border-radius:15px; -moz-border-radius:15px; padding:1%; border:2px solid #eee; width:auto; height:auto; text-align:center; font-size:150%; }\n";
...
function adrotate_welcome_pointer() {
    ...
    $pointer_content .= '<p>Welcome, and thanks for using AdRotate. Everything related to AdRotate is in this menu. Check out the <a href="http:\/\/www.adrotateplugin.com\/support\/knowledgebase\/" target="_blank">knowledgebase</a> and <a href="http:\/\/www.adrotateplugin.com\/support\/forums\/" target="_blank">forums</a> if you get stuck with something. You can also use <a href="admin.php?page=adrotate-support">ticket support</a> if you have a valid license.</p>';
?>
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			$('#toplevel_page_adrotate').pointer({
				...
				close: function() {
	                $.post(ajaxurl, {
	                    pointer:'adrotate_pro_'+<?php echo ADROTATE_VERSION.ADROTATE_DB_VERSION; ?>,
...
function adrotate_credits() {
	...
	echo __('Find my website at', 'adrotate').' <a href="http://meandmymac.net" target="_blank">meandmymac.net</a>.<br />';
	...
	echo '<td><a href="https://ajdg.solutions/" title="AJdG Solutions">AJdG Solutions</a> - '.__('Your one stop for Webdevelopment, consultancy and anything WordPress! Find out more about what I can do for you!', 'adrotate').' '.__('Visit the', 'adrotate').' <a href="https://ajdg.solutions/" target="_blank">'.__('AJdG Solutions', 'adrotate').'</a> '.__('website', 'adrotate').'.</li>';
	echo '</ul></td>';
	echo '</tr>';
	echo '</tbody>';

	echo '</table>';
	echo '<center><small>AdRotate&trade; and the AdRotate Logo (including color variations) are owned by Arnan de Gans for AJdG Solutions.</small></center>';
}
