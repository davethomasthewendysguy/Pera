...
text-align:left;",
		...
		function JustifiedImageGrid($case = false){
			...
			$this->settings_override = array(	'fb_app_id' 				=> $this->settings['fb_app_id'],
												...
												'fb_add_friends'			=> $this->settings['fb_add_friends'],
												...
												'flickr_collections_slug'	=> $this->settings['flickr_collections_slug'],
												'flickr_breadcrumb_css'		=> $this->settings['flickr_breadcrumb_css'],
												...
												'image_custom_classes'		=> $this->settings['image_custom_classes'],
												...
												'show_up_in_feeds'			=> $this->settings['show_up_in_feeds'],
...
\t<link rel='canonical' href='".$og_url_meta_for_fb."' />";
		...
		function jig_build_settings_page(){
			...
			<script type="text/javascript">
					...
					jQuery(document).ready(function(jQuery) {
						...
						addMiniSection('link_class',"<?php _e('Link attributes (custom lightbox)', 'jig_td'); ?>");
	...
	<?php
		...
		function jig_init_options(){
			...
			// WP image custom classes
			add_settings_field(
				'jig_image_custom_classes',
				__('WP image custom classes', 'jig_td'),
				array($this, 'jig_print_general_input'),
					self::PAGE_NAME,
					'jig_general_settings_section',
					array(	'id' => 'image_custom_classes',
							...
							'inputs' => array(
								'disable' => __('Disable, do not change anything.', 'jig_td'),
								'enable' => __('Enable the ability to add a custom class to each image.', 'jig_td')
							)
					)
			);
			// Media re-attacher
			...
			add_settings_field(
				...
				array($this, 'jig_print_text_input'),
					...
					array(	'id' => 'pp_social_buttons',
							'label' => __('Default is FTPG. Toggle individual social buttons or re-order them. One letter means one social button.<br/>F = Facebook Like, T = Twitter, P = Pinterest, G = Google+', 'jig_td')
					)
			);
			...
			// Add your friends as pages
			add_settings_field(
				'jig_fb_add_friends',
				__('Add your friends as pages', 'jig_td'),
				array($this, 'jig_print_general_input'),
					self::PAGE_NAME,
					'jig_facebook_section',
					array(	'id' => 'fb_add_friends',
							'help' => __("The use of this feature is at your own risk, the plugin author is not liable for any privacy issues that arise from publishing photos that are originally only shared with you. Please respect your friends' privacy. This setting allows you to add your friends by entering their profile URL to the 'Add a new Facebook page' button/field. Once you have activated this setting (and saved) you need to click 'Add current Facebook user' once again to give your APP permission to access the friends photos. This does not work for every user, probably depending on their privacy settings.", 'jig_td'),
							'inputs' => array(
								'no' => __("No: Require your friends' authorization, permission and 2-step process to add them.", 'jig_td'),
								'yes' => __('Yes: Using friends photos permission, add them much like restricted pages with access via your user.', 'jig_td')
							)
					)
			);
			// --------------------------------
		...
		function jig_print_fb_authed(){
			...
			$output .= '<div id="jigFbAuthOtherUserPanel">
							...
							<input type="text" id="jigFbOtherUserLink" value="" data-force="https://www.facebook.com/dialog/oauth?client_id='.trim($this->settings['fb_app_id']).'&scope=user_photos' . ($this->settings['fb_add_friends'] == 'no' ? '' : ',friends_photos') . '&redirect_uri='.urlencode(plugins_url('fb-auth-other-user.php', __FILE__)).'" /><br />
		...
		*/
		...
		function jig_init_shortcode($atts){
			...
			switch($gallery_type){
					...
					if($attachments){ // If there are images attached to the post 
						...
						foreach ($attachments as $attachment){ // Loop through each
							...
							if($d['link'] != '') {
								...
								if($lightbox == 'foobox' && $data['link_target'] == 'video'){
									$data['link_target'] = 'foobox';
								}
							...
							if($this->settings['image_custom_classes'] === 'enable'){
								...
								if($d['extra_class'] != '') $data['extra_class'] = $d['extra_class'];
							}
				...
				case 'wp_recent_posts':
					...
					if(!empty($recents_author)){
						if($recents_author !== 'currently_logged_in'){
							$recents_user = get_userdatabylogin($recents_author);
							...
							if($recents_current_user_id !== 0){
								...
								return sprintf(__('You need to be logged in to view your posts.', 'jig_td'),$id);
							}
						}
					}
					...
					if ($posts){ // If there are images attached to the post  
						...
						foreach ($posts as $post){ // Loop through each
								...
								switch($recents_description_value){
									...
									case 'author':
										if($author_prefix !== 'none'){
											$d['description'][$recents_desc_id] = $author_prefix.get_userdata($post->post_author)->user_login;
										}else{
											$d['description'][$recents_desc_id] = get_userdata($post->post_author)->user_login;
										}
							...
							if($this->settings['image_custom_classes'] === 'enable'){
								$d['extra_class'][] = esc_attr(stripslashes(get_post_meta(get_post_thumbnail_id($post->ID), '_jig_custom_class', true)));
							}
												...
												if($ng_lightbox_gallery == 'yes'){ // If gallery should be displayed as a lightbox
													...
													if(!empty($lightbox_images)){
														...
														foreach ($lightbox_images as $lightbox_image) {
															...
															if($link_class){
																$shadow_class = 'class="'.$link_class.'" ';
															}else{
																$shadow_class = '';
															}
															$shadow_gallery .= '<a href="'.$data['url'].'" rel="'.$shadow_rel.'"'.$shadow_class.' title="'.$title_fragment.'" class="jig-link"><img src="data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D" alt="'.$alt_fragment.'" /></a>';
														}
														$shadow_gallery .= "</div>";
													}
												}
				...
				case 'facebook':
					if(isset($this->settings['fb_authed'][$facebook_id])){
						...
						if(strpos($facebook_album,',') !== false){ // Multi album mode
						...
						if($facebook_album == 'overview' || $facebook_album == 'overview_only_albums'){
							$albums_url = "https://graph.facebook.com/".$facebook_id."/albums?fields=id,cover_photo,link,count,name,description,type,from".$limit_parameter.($user['access_token'] != 'public' ? '&access_token='.$user['access_token'] : '');
								...
								if(get_query_var($this->settings['fb_overview_slug'])){					
									...
									foreach ($albums->data as $key => $album) {
										if($facebook_album == 'overview_only_albums' && $album->type !== 'normal'){
											continue;
										}elseif(!empty($facebook_album_multi) && !in_array($album->id, $facebook_album_multi)){
											continue;
										}
										...
										if(!empty($album->count) && !empty($album->link)){
											if($fb_lightbox_album == 'yes'){
												...
												// Since FB has a cover_photo in the album, it can be queried directly, saving some data on the request. This is just one single photo.
												if(!empty($album->cover_photo)){
													$subalbum_url = "https://graph.facebook.com/".$album->cover_photo."?fields=source,height,width,images".($user['access_token'] != 'public' ? '&access_token='.$user['access_token'] : '');
												}else{
													// If the cover photo is not set then JIG will find the first image and use that
											...
											// Faking an album call, when the call was only for a single cover picture
											if(!empty($subalbum->images) && empty($subalbum->data)){
												$subalbum->data[] = $subalbum;
											}
											...
											if(!empty($subalbum->data)){
												...
												if($facebook_count == 'yes'){
													$description_fragments = array($album->count.' '._n('Photo', 'Photos', $album->count, 'jig_td'));
												...
												if($fb_lightbox_album == 'yes'){ // If gallery should be displayed as a lightbox
													...
													foreach ($subalbum->data as $subalbum_image) {
														...
														if($subalbum_image->id == $subalbum->data[0]->id){
															...
															if($download_link != 'no'){
																$data['download'] = esc_attr(stripslashes('<a href="'.plugins_url('download.php', __FILE__).'?file='.$shadow_data['url'].'">'.$this->settings['download_link_text'].'</a>'));
															}
														...
														$shadow_gallery .= '<a href="'.$shadow_data['url'].'" rel="'.$shadow_rel.'"'.$shadow_class.' title="'.$title_fragment.'" class="jig-link"><img src="data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D" alt="'.$alt_fragment.'" /></a>';
								...
								foreach ($photos->data as &$feed_post) {
									if(!(isset($feed_post->picture) && isset($feed_post->object_id))){
										...
										if(isset($photo_object->images)){
											...
											if($download_link != 'no'){
												$data['download'] = esc_attr(stripslashes('<a href="'.plugins_url('download.php', __FILE__).'?file='.($data['link'] ? $data['link'] : $data['url']).'">'.$this->settings['download_link_text'].'</a>'));
											}
											array_push($this->images, $data); // Add to the main images array
										}									
									}
						...
						if($facebook_album !== 'overview' && $facebook_album !== 'overview_only_albums' && $facebook_album !== 'feed'){
							...
							if(!empty($photos->data)){
								...
								if(($facebook_description == 'yes' || $facebook_description == 'above') && empty($facebook_description_displayed_already)){
									$facebook_album_url = "https://graph.facebook.com/".$facebook_album."?fields=description".($user['access_token'] != 'public' ? '&access_token='.$user['access_token'] : '');
									if($facebook_caching > 0){
				...
				case 'flickr':
					...
					if($flickr_collection !== ''){
						...
						if($flickr_collection !== 'complete-overview'){
							$collections_url = 'http://api.flickr.com/services/rest?api_key='.trim($this->settings['fli_api_key']).'&format=php_serial&method=flickr.collections.getTree&collection_id='.$flickr_collection.'&user_id='.$flickr_user;
						}else{
							$collections_url = 'http://api.flickr.com/services/rest?api_key='.trim($this->settings['fli_api_key']).'&format=php_serial&method=flickr.collections.getTree&user_id='.$flickr_user;
						}
						if($flickr_caching > 0){
						...
						if($collections_raw['stat'] == 'ok'){
							...
							if($flickr_set_list_needed === true){
								$flickr_set_list_url = 'http://api.flickr.com/services/rest?api_key='.trim($this->settings['fli_api_key']).'&format=php_serial&method=flickr.photosets.getList&user_id='.$flickr_user.'&per_page=500&primary_photo_extras=description,url_k,url_h,url_l,url_c,url_z,url_m,url_n,url_s,url_t';
								if($flickr_caching > 0){
						...
						if($flickr_photoset === '' && (count($collections[$flickr_collection]['set']) > 0 || count($collections[$flickr_collection]['collection']) > 0)){
							$this->images = array(); // Create a new array for the images
							if(count($collections[$flickr_collection]['set']) > 0){
								...
								foreach ($collections[$flickr_collection]['set'] as $single_set) {
									...
									if($flickr_lightbox_set == 'yes'){ // If gallery should be displayed as a lightbox
										...
										$shadow_galleries[] = $shadow_group_id = 'jigHiddenGalleryGroup-'.$single_set['id'];
										$shadow_gallery = '<div class="jigHiddenGallery">';
										switch($lightbox){
											case 'prettyphoto':
											...
											break;
											default:
											$shadow_rel = 'fli['.$single_set['id'].']';
											break;
										}

										$photoset_url = 'http://api.flickr.com/services/rest?api_key='.trim($this->settings['fli_api_key']).'&format=php_serial&method=flickr.photosets.getPhotos&photoset_id='.$single_set['id'].$limit_parameter.'&extras=description,tags,geo,url_k,url_h,url_l,url_c,url_z,url_m,url_n,url_s,url_t';
										...
										if($single_photoset['stat'] == "ok"){
											foreach ($single_photoset['photoset']['photo'] as $shadow_photo) {
												...
												if($shadow_photo['id'] == $single_set['primary']){
													...
													if($flickr_link != 'no'){
														$data['lightbox_link'] = esc_attr(stripslashes('<a href="http://www.flickr.com/photos/'.$flickr_user.'/'.$shadow_photo['id'].'" target="'.$this->settings['flickr_link_target'].'">'.$this->settings['flickr_link_text'].'</a>'));
														}
												...
												if($flickr_link != 'no'){
													$shadow_data['lightbox_link'] = esc_attr(stripslashes('<a href="http://www.flickr.com/photos/'.$flickr_user.'/'.$shadow_photo['id'].'" target="'.$this->settings['flickr_link_target'].'">'.$this->settings['flickr_link_text'].'</a>'));
													if($flickr_link == 'yes'){
														if($title_fragment !== ''){
							...
							}elseif(count($collections[$flickr_collection]['collection']) > 0){
								...
								foreach ($collections[$flickr_collection]['collection'] as $single_collection) {
									...
									if($flickr_count == 'yes'){
										...
										if(count($single_collection['set']) > 0){
					...
					$photos_url = 'http://api.flickr.com/services/rest?api_key='.trim($this->settings['fli_api_key']).'&format=php_serial&method='.$photos_url_bit.$limit_parameter.'&extras=description,tags,geo,url_k,url_h,url_l,url_c,url_z,url_m,url_n,url_s,url_t';
					...
					if($photos['stat'] == "ok"){
						...
						if((isset($photos['photos']) && count($photos['photos']['photo']) > 0)
							...
							foreach ($photos['photos']['photo'] as $photo) {
								...
								if($flickr_link != 'no'){
									$data['lightbox_link'] = esc_attr(stripslashes('<a href="http://www.flickr.com/photos/'.($photo['owner'] ? $photo['owner'] : $flickr_user).'/'.$photo['id'].'" target="'.$this->settings['flickr_link_target'].'">'.$this->settings['flickr_link_text'].'</a>'));
								}
								if($filterby == 'on'){
							...
							if($flickr_search_text !== '' || $flickr_search_tags !== ''){
								return __('There are no photos that match your search criteria.', 'jig_td');
							}else{
			...
			switch($lightbox){
				case 'prettyphoto':
        			if(!defined('JIG_SKIP_PRETTYPHOTO')){
						...
						wp_register_script('jig-prettyphoto', plugins_url('js/jquery.prettyphoto.custom-min.js', __FILE__), 'jquery', '3.1.5.1.8', true);
						wp_enqueue_script('jig-prettyphoto');
						wp_register_style('jig-prettyphoto-style', plugins_url('css/prettyphoto.css', __FILE__), false, '3.1.5.1.8');
			...
			foreach ($this->images as $image_index => &$image_element) {
				...
				$noscript_output .= '<a href="'.(!isset($image_element['link']) ? $image_element['url'] : $image_element['link']).'"'.$title_fragment.$rel_fragment.'><img src="'.str_replace('&','&amp;',$image_src).'" alt="'.$alt_fragment.'" width="'.(!isset($image_element_original_width) ? $image_element['width'] : $image_element_original_width).'" height="'.$max_height.'" /></a>';
				if($this->settings['show_up_in_feeds'] == 'yes' && is_feed()){
					...
					if(strrpos($image_rss_src, '.') > 2){
						$ext = "&f=".substr($image_rss_src, strrpos($image_rss_src, '.'));
					}
			...
			if($filterby !== 'off' && !empty($filters_for_JS)){
				...
				if($filter_all_button == 'yes'){
					...
					$filters_for_JS['all-items-nofilter'] = $filter_all_text;
				}
			...
			wp_enqueue_script('justified-image-grid', plugins_url('js/justified-image-grid-min.js', __FILE__), 'jquery', '1.8', true);
			return $output;
		...
		function jig_fb_auth(){
			...
			if(empty($_REQUEST["code"])) {
				...
				. $app_id . "&scope=user_photos" . ($this->settings['fb_add_friends'] == 'no' ? '' : ',friends_photos') . "&redirect_uri=" . urlencode($redirect_uri) . "&state="
				. $jig_session['state'];
		...
		function jig_add_fb_page($token = '', $user_name = ''){
			...
			if($page != ''){
				...
				if(!empty($albums->data)){
					...
					$try_as_friend = $this->settings['fb_add_friends'] == 'yes' && !empty($main->first_name);
					if($main === false || $main->error->message == "Unsupported get request." || $main->error->code == 4 || $try_as_friend){
						$options = array();
						if(isset($this->settings['fb_authed'])){
							...
							if($options_count == 1){
								...
								if($token != '' ){
									$output = array('error' => sprintf(__("Demographically or geographically blocked for %1$s, maybe try again with another user. If you are adding a friend's profile using 'Add your friends as pages' setting then you need to click on 'Add current Facebook user' again.", 'jig_td'), $user_name));
								}else{
						...
						if(!empty($main->first_name)){
							...
							$output = array('error' => __('No albums or SSL certificate problem, verify that the CA cert is OK: <a href="http://snippets.webaware.com.au/howto/stop-turning-off-curlopt_ssl_verifypeer-and-fix-your-php-config/" target="_blank">check this out for more information</a>', 'jig_td'));
						}					
					}
				}
			}else{
		...
		function jig_get_fli_types(){
			...
			$flickr_url = "http://api.flickr.com/services/rest?api_key=".$fli_api_key."&format=php_serial&method=flickr.collections.getTree&per_page=1&user_id=".$user_id;
			$rsp = unserialize($this->file_get_contents_curl($flickr_url));
		...
		function jig_get_fli_elements(){
			...
			switch($type){
				...
				break;
				case 'collection':
					$flickr_url = "http://api.flickr.com/services/rest?api_key=".$fli_api_key."&format=php_serial&method=flickr.collections.getTree&user_id=".$user_id;
					$rsp = unserialize($this->file_get_contents_curl($flickr_url));
					if(count($rsp['collections']) > 0){
		...
		function flickr_collection_walk($collection,$fli_api_key,$depth){
			...
			if(count($collection['collection']) > 0){
				...
				foreach ($collection['set'] as $set) {
					...
					$flickr_url = "http://api.flickr.com/services/rest?api_key=".$fli_api_key."&format=php_serial&method=flickr.photosets.getInfo&photoset_id=".$set['id'];
					$set_rsp = unserialize($this->file_get_contents_curl($flickr_url));

					if(isset($set_rsp['stat']) && $set_rsp['stat'] == 'ok'){
		...
		function jig_ng_process_images($images){
			...
			foreach ( $images as &$image) {
				...
				if(defined('WINABSPATH') && strstr($image->path,WINABSPATH) !== false){
					$image->path = substr($image->path,strlen(WINABSPATH));
				}
		...
		function jig_ng_get_galleries($gallery_ids, $order_by = 'sortorder', $order_dir = 'ASC', $exclude = true, $limit = 0, $force_nonnumeric = false) {
			global $wpdb;
			...
			if(empty($images) && is_numeric($gallery_ids) && $force_nonnumeric == false){
				$images = $this->jig_ng_get_galleries($gallery_ids, $order_by, $order_dir, $exclude, $limit, true);
				return $images;
			}
			if(!empty($images)){
				$images = $this->jig_ng_process_images($images); // Very important, sets up the image objects, mimics NG
