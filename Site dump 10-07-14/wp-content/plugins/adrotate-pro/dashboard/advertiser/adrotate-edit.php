	...
	if($edit_banner->id == $advertiser) {
		if($ad_edit_id AND $edit_banner->type != 'a_empty') {
			...
			if(!preg_match_all('/<a[^>](.*?)>/i', stripslashes(htmlspecialchars_decode($edit_banner->bannercode, ENT_QUOTES)), $things) AND $edit_banner->tracker == 'Y' AND $adrotate_config['clicktracking'] == 'Y')
				echo '<div class="error"><p>'. __("jQuery clicktracking is enabled but no valid link was found in the adcode!", 'adrotate') .' '. __("Please contact staff if you don't know what this means.", 'adrotate').'</p></div>';

			if(!preg_match("/%link%/i", $edit_banner->bannercode) AND $edit_banner->tracker == 'Y' AND $adrotate_config['clicktracking'] == 'N')
				echo '<div class="error"><p>'. __("Redirect clicktracking is enabled but the %link% tag was not found in the adcode!", 'adrotate') .' '. __("Please contact staff if you don't know what this means.", 'adrotate').'</p></div>';
		...
		<?php } ?>
			...
			<table class="widefat" style="margin-top: .5em">
				...
				<tbody>
			  	...
			  	<?php if($adrotate_config['clicktracking'] == 'N') { ?>
		      	<tr>
			        <th valign="top"><?php _e('Target URL:', 'adrotate'); ?></th>
			        <td colspan="3">
			        	<label for="adrotate_link"><input tabindex="5" name="adrotate_link" type="text" size="80" class="search-input" value="<?php echo $edit_banner->link;?>" /><br /><em><?php _e('Place the target URL in the field, use %link% in the adcode instead of the target URL - Similar to code example 2.', 'adrotate'); ?></em></label>
			        </td>
		      	</tr>
		      	<?php } ?>
				<tr>
