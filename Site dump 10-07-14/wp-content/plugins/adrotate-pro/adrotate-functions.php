...
function adrotate_filter_schedule($selected, $banner) { 
	...
	$schedules = $wpdb->get_results("SELECT `".$prefix."adrotate_schedule`.`id`, `starttime`, `stoptime`, `maxclicks`, `maximpressions`, `spread`, `hourimpressions` FROM `".$prefix."adrotate_schedule`, `".$prefix."adrotate_linkmeta` WHERE `schedule` = `".$prefix."adrotate_schedule`.`id` AND `ad` = ".$banner->id." ORDER BY `starttime` ASC;");
	...
	foreach($schedules as $schedule) {	
		if($schedule->starttime > $now OR $schedule->stoptime < $now) {
			...
			if($adrotate_config['enable_stats'] == 'Y') {
				$stat = adrotate_stats($banner->id, $schedule->starttime, $schedule->stoptime, true);
				...
				if($adrotate_debug['general'] == true) {
					...
					echo "<br />Impressions last hour: ".$stat['hour'];
					echo "<br />Impression Spread: ".$schedule->spread."(Max ".$schedule->hourimpressions." per hour)";
					echo "</pre></p>";
				}
				...
				if($schedule->spread == 'Y') {
					if($stat['hour'] >= $schedule->hourimpressions) {
...
function adrotate_mail_notifications() {
	...
	if($notifications['notification_email'] == 'Y' AND $notifications['notification_email_switch'] == 'Y') {
		...
		if($advert_status['total'] > 0 OR $advert_status['test']) {
			...
			if($advert_status['test']) $message .= "<p><strong>This is a test notification!</strong></p>";
...
function adrotate_dashboard_scripts() {
	...
	$do_add_script = false;
	if(!in_array('adrotate_pro_'.ADROTATE_VERSION.ADROTATE_DB_VERSION, $seen_it)) {
		$do_add_script = true;
...
function adrotate_status($status, $args = null) {
	...
	switch($status) {
		...
		case '611' :
			echo '<div id="message" class="updated"><p>This license can not be activated for networks. Please purchase a Developer or Network license.</p></div>';
		break;
