...
function woo_shortcode_footer_woothemes_link ( $atts ) {
	...
	$gm_link = 'http://www.gourmetmarketing.net/';
	if ( isset( $woo_options['woo_footer_aff_link'] ) && '' != $woo_options['woo_footer_aff_link'] ) {
		$gm_link = $woo_options['woo_footer_aff_link'];
	}
	
	$output = sprintf( '%s<a href="%s" title="%s" alt="%s" class="gourmet-marketing-badge" target="_blank">%s</a>%s', $atts['before'], esc_url( $gm_link ), 'Restaurant Marketing Made Easy', 'Restaurant Marketing Made Easy', '<img src="' . esc_url( get_template_directory_uri() . '/images/gourmet-marketing.png' ) . '" width="95" height="28" alt="Restaurant Marketing Made Easy! Restaurant Marketing Plans by Gourmet Marketing. Find us on www.gourmetmarketing.net" />', $atts['after'] );
...
function woo_shortcode_site_credit ( $atts ) {
	$defaults = array(
		'before' => '<p class="gourmet-marketing-credit">',
		'after' => '</p>'
	);
	$atts = shortcode_atts( $defaults, $atts );
	
	$output = sprintf( '%1$s %4$s %5$s %3$s %2$s', $atts['before'], $atts['after'], '<a class="gourmet-marketing-text" href="http://www.gourmetmarketing.net/services/restaurant-website-design/" title="Restaurant Website Design by Restaurant Website Designers. Visit us on http://www.gourmetmarketing.net/services/restaurant-website-design/" target="_blank">' . __( 'Restaurant Website Design by Gourmet Marketing', 'woothemes' ) . '</a>', '[footer_woothemes_link]', '<span class="gourmet-marketing-sep"></span>' );
...
/**
 * 19. Extended Social Icons
 *
 * This function produces the default footer credit text.
 * 
 * @example <code>[social_icon]</code> is the default usage
 */
function woo_shortcode_socialicon_extended ( $atts, $content = null ) {
	$defaults = array( 'url' => '', 'float' => 'none', 'icon_url' => '', 'title' => '', 'profile_type' => '', 'window' => 'no', 'rel' => '', 'classes' => '' );
	...
	$output = '<a href="' . esc_attr( $url ) . '" title="' . esc_attr( $_alt_text ) . '"' . $target . $rel . ' class="' . esc_attr( $_classes ) . '">';
	
	if ($_img_url)	
		$output .= '<img src="' . esc_url( $_img_url ) . '" alt="' . esc_attr( $_alt_text ) . '" />';
	
	$output .= '</a>' . "\n";

	return $output;
} // End woo_shortcode_socialicon_extended()

remove_shortcode( 'social_icon', 'woo_shortcode_socialicon' );
add_shortcode( 'social_icon', 'woo_shortcode_socialicon_extended' );
